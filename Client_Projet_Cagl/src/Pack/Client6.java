	...
	public static void main(String[] args) {
		try {
			while(true) {
				...
				/*------------------ receive jeton from client5(UDP)----------------------------*/
				DatagramSocket c = new DatagramSocket(1006);
				byte[] receiveData = new byte[1024];
				DatagramPacket in_client5 = new DatagramPacket(receiveData, receiveData.length);
				c.receive(in_client5);
				String jeton = new String(in_client5.getData());
				
				/*------------------ receive jeton from client5(UDP)----------------------------*/
				
				if (jeton.trim().equals("jeton")) {
					System.out.println("jeton recu par le client6...."+jeton);
                ...
                String M;
                String request = "";
                    System.out.println("****************MENU SERVICE Client6******************");
            ...
            System.out.println();
            String chaineRefClient6 = "S5, S4, S1, S3, S10, S7, S6, S0, S13, S2, S1, S7, S6, S3, S15, S2, S1, S14, S5, S6. FIN";
            
            try{
    			Registry reg = LocateRegistry.getRegistry("localhost",3333);
    			InterfaceInter i = (InterfaceInter) reg.lookup("Refs");
    			String ACK;
    			ACK = i.processRequest(6,chaineRefClient6, request);
	            ...
	            DatagramPacket out_client1 = new DatagramPacket(sendJeton, sendJeton.length, ip, 1001);
	            System.out.println("client6 passe le jeton to client 1...");
	            C.send(out_client1);
	            /*------------------ send jeton To client6(UDP)----------------------------*/
	...
	public void startTimer() {
	    ActionListener taskPerformer = new ActionListener() {
	        public void actionPerformed(ActionEvent evt) {
	            ...
	            System.out.println("Le délai d'attente est écoulé. le client5 n'envoie pas le jeton....");
	            ...
	            try {
		            ...
		            if (value == 6) {
			            ...
			            InetAddress ip = InetAddress.getByName("localhost");
			            DatagramPacket out_client1 = new DatagramPacket(sendJeton, sendJeton.length, ip, 1001);
			            System.out.println("client6 passe le nouveau jeton to client 1...");
			            C.send(out_client1);
			            /*------------------ send jeton To client3(UDP)----------------------------*/
			            C.close();
		            } else {
