	...
	public static void main(String[] args) {
		try {
			while(true) {
				...
				/*------------------ receive jeton from client3(UDP)----------------------------*/
				DatagramSocket c = new DatagramSocket(1004);
				byte[] receiveData = new byte[1024];
				DatagramPacket in_client3 = new DatagramPacket(receiveData, receiveData.length);
				c.receive(in_client3);
				String jeton = new String(in_client3.getData());
                ...
                String M;
                String request = "";
                    System.out.println("****************MENU SERVICE Client4******************");
            ...
            System.out.println();
            String chaineRefClient4 = "S13, S2, S1, S7, S0, S4, S6, S5, S11, S0, S14, S3, S9, S1, S0, S3, S6, S7, S12, S11, FIN";
            
            try{
    			Registry reg = LocateRegistry.getRegistry("localhost",3333);
    			InterfaceInter i = (InterfaceInter) reg.lookup("Refs");
    			String ACK;
    			ACK = i.processRequest(4,chaineRefClient4, request);
		        ...
		        /*------------------ send jeton To client5(UDP)----------------------------*/
		        DatagramSocket C = new DatagramSocket();
		        jeton = "jeton";
	            byte[] sendJeton = jeton.getBytes();
	            InetAddress ip = InetAddress.getByName("localhost");
	            DatagramPacket out_client5 = new DatagramPacket(sendJeton, sendJeton.length, ip, 1005);
	            System.out.println("client4 passe le jeton to client 5...");
	            C.send(out_client5);
	            /*------------------ send jeton To client5(UDP)----------------------------*/
	...
	public void startTimer() {
	    ActionListener taskPerformer = new ActionListener() {
	        public void actionPerformed(ActionEvent evt) {
	            ...
	            System.out.println("Le délai d'attente est écoulé. le client3 n'envoie pas le jeton....");
	            ...
	            try {
		            ...
		            if (value == 6) {
			            ...
			            InetAddress ip = InetAddress.getByName("localhost");
			            DatagramPacket out_client5 = new DatagramPacket(sendJeton, sendJeton.length, ip, 1005);
			            System.out.println("client4 passe le nouveau jeton to client 5...");
			            C.send(out_client5);
			            /*------------------ send jeton To client3(UDP)----------------------------*/
			            C.close();
