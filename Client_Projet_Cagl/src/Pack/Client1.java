...
public class Client1{
	...
	public static void main(String[] args) {
		try {
		    ...
		    while(true) {
                ...
                String M;
                String request = "";
                    System.out.println("****************MENU SERVICE Client1******************");
            ...
            System.out.println("------->Client1 a demmandé le service"+request+"<-------");
            System.out.println();
            String chaineRefClient1 = "S7, S10, S1, S2, S4, S3, S0, S6, S12, S11, S8, S5, S2, S1, S0, S5, S1, S7, S9, S2.FIN";
            
            try{
    			Registry reg = LocateRegistry.getRegistry("localhost",3333);
    			InterfaceInter i = (InterfaceInter) reg.lookup("Refs");
    			String ACK;
    			ACK = i.processRequest(1,chaineRefClient1, request);
		        ...
		        /*------------------ send jeton To client2(UDP)----------------------------*/
		        DatagramSocket c = new DatagramSocket();
	            String jeton = "jeton";
	            byte[] sendJeton = jeton.getBytes();
	            InetAddress ip = InetAddress.getByName("localhost");
	            DatagramPacket out_client2 = new DatagramPacket(sendJeton, sendJeton.length, ip, 1002);
	            System.out.println("client1 passe le jeton to client 2...");
	            c.send(out_client2);
	            /*------------------ send jeton To client2(UDP)----------------------------*/
	            
	            
	            
	            // Initialisation du timer pour atteindre que le client6 envoie le jeton
	            ...
	            client.startTimer();
			     ...
			     /*------------------ receive jeton from client6(UDP)----------------------------*/
			     DatagramSocket C = new DatagramSocket(1001);
				 byte[] receiveData = new byte[1024];
				 DatagramPacket in_client6 = new DatagramPacket(receiveData, receiveData.length);
				 C.receive(in_client6);
				 String Jeton = new String(in_client6.getData());
				...
				/*------------------ receive jeton from client6(UDP)----------------------------*/
				 ...
				 if (Jeton.trim().equals("jeton")) {
						System.out.println("jeton recu par le client1...."+Jeton);
				...
				Thread.sleep(5000);
					...
					// Arrêt du Timer après la réception du jeton à partir de client6
		            client.stopTimer();
    ...
    ActionListener taskPerformer = new ActionListener() {
        public void actionPerformed(ActionEvent evt) {
        	...
        	System.out.println("Le délai d'attente est écoulé. le client6 n'envoie pas le jeton....");
    ...
    // Exemple de délai de 10 secondes (ajustez selon vos besoins)
    int delay = 120000;
