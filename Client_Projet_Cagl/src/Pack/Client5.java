	...
	public static void main(String[] args) {
		try {
			while(true) {
				...
				/*------------------ receive jeton from client4(UDP)----------------------------*/
				DatagramSocket c = new DatagramSocket(1005);
				byte[] receiveData = new byte[1024];
				DatagramPacket in_client4 = new DatagramPacket(receiveData, receiveData.length);
				c.receive(in_client4);
				String jeton = new String(in_client4.getData());
				...
				if (jeton.trim().equals("jeton")) {
					System.out.println("jeton recu par le client5...."+jeton);
                ...
                String M;
                String request = "";
                    System.out.println("****************MENU SERVICE Client5******************");
            ...
            System.out.println();
            String chaineRefClient5 = "S2, S14, S1, S7, S6, S3, S0, S5, S2, S15, S7, S0, S4, S12, S3, S6, S7, S2, S1, S13. FIN";
            
            try{
    			Registry reg = LocateRegistry.getRegistry("localhost",3333);
    			InterfaceInter i = (InterfaceInter) reg.lookup("Refs");
    			String ACK;
    			ACK = i.processRequest(5,chaineRefClient5, request);
		        ...
		        /*------------------ send jeton To client6(UDP)----------------------------*/
		        DatagramSocket C = new DatagramSocket();
		        jeton = "jeton";
	            byte[] sendJeton = jeton.getBytes();
	            InetAddress ip = InetAddress.getByName("localhost");
	            DatagramPacket out_client6 = new DatagramPacket(sendJeton, sendJeton.length, ip, 1006);
	            System.out.println("client5 passe le jeton to client 6...");
	            C.send(out_client6);
	            /*------------------ send jeton To client6(UDP)----------------------------*/
	...
	public void startTimer() {
	    ActionListener taskPerformer = new ActionListener() {
	        public void actionPerformed(ActionEvent evt) {
	            ...
	            System.out.println("Le délai d'attente est écoulé. le client4 n'envoie pas le jeton....");
	            ...
	            try {
		            ...
		            if (value == 6) {
			            ...
			            InetAddress ip = InetAddress.getByName("localhost");
			            DatagramPacket out_client6 = new DatagramPacket(sendJeton, sendJeton.length, ip, 1006);
			            System.out.println("client5 passe le nouveau jeton to client 6...");
			            C.send(out_client6);
			            /*------------------ send jeton To client3(UDP)----------------------------*/
			            C.close();
		            } else {
